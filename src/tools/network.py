...
@server.tool(
    ...
    description=(
        "List all configured networks (LAN, WAN, VLAN-only) on the Unifi Network "
        "controller (V1 API based)."
    )
)
async def list_networks() -> Dict[str, Any]:
    ...
    if not parse_permission(permissions, "network", "read"):
...
async def get_network_details(network_id: str) -> Dict[str, Any]:
    ...
    permissions = getattr(config, 'permissions', None) or {}
    if not parse_permission(permissions, "network", "read"):
        logger.warning("Permission denied for getting network details (%s).", network_id)
...
@server.tool(
    ...
    description=(
        "Update specific fields of an existing network (LAN/VLAN). "
...
async def update_network(
    ...
    """
    permissions = getattr(config, 'permissions', None) or {}
    if not parse_permission(permissions, "network", "update"):
        logger.warning("Permission denied for updating network (%s).", network_id)
        return {"success": False, "error": "Permission denied to update network."}

    if not confirm:
        logger.warning("Confirmation missing for updating network %s.", network_id)
...
async def create_network(
    ...
    """
    permissions = getattr(config, 'permissions', None) or {}
    if not parse_permission(permissions, "network", "create"):
    ...
    if not is_valid or not isinstance(validated_data, dict):
    ...
    logger.info("Attempting to create network '%s' with purpose '%s'", validated_data.get('name'), purpose)
    try:
...
async def list_wlans() -> Dict[str, Any]:
    ...
    """
    permissions = getattr(config, 'permissions', None) or {}
    if not parse_permission(permissions, "wlan", "read"):
...
async def get_wlan_details(wlan_id: str) -> Dict[str, Any]:
    ...
    permissions = getattr(config, 'permissions', None) or {}
    if not parse_permission(permissions, "wlan", "read"):
        logger.warning("Permission denied for getting WLAN details (%s).", wlan_id)
...
async def update_wlan(
    ...
    """
    permissions = getattr(config, 'permissions', None) or {}
    if not parse_permission(permissions, "wlan", "update"):
        logger.warning("Permission denied for updating WLAN (%s).", wlan_id)
        return {"success": False, "error": "Permission denied to update WLAN."}

    if not confirm:
        logger.warning("Confirmation missing for updating WLAN %s.", wlan_id)
        return {"success": False, "error": "Confirmation required. Set 'confirm' to true."}
...
async def create_wlan(
    ...
    logger.info("Attempting to create WLAN '%s' with security '%s'", validated_data.get('name'), validated_data.get('security'))
    try:
        ...
        if created_wlan and isinstance(created_wlan, dict) and created_wlan.get('_id'):
            ...
            site = getattr(network_manager, '_connection', None)
            site_id = getattr(site, 'site', None) if site else None
            logger.info("Successfully created WLAN '%s' with ID %s", validated_data.get('name'), new_wlan_id)
            return {
                "success": True,
                "site": site_id,
